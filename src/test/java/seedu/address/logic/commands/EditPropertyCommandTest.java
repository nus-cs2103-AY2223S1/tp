...
class EditPropertyCommandTest {
    ...
    void execute_someFieldsSpecifiedUnfilteredList_success() {
        ...
        Property editedProperty = propertyInList.withName(VALID_NAME_PROPERTY1).withPrice(VALID_PRICE_PROPERTY1).build();
    ...
    public void execute_noFieldSpecifiedUnfilteredList_success() {
        EditPropertyCommand editPropertyCommand = new EditPropertyCommand(INDEX_FIRST_ITEM, new EditPropertyDescriptor());
        Property editedProperty = model.getFilteredPropertyList().get(INDEX_FIRST_ITEM.getZeroBased());

        String expectedMessage = String.format(EditPropertyCommand.MESSAGE_EDIT_PROPERTY_SUCCESS, editedProperty);

        Model expectedModel = new ModelManager(new AddressBook(model.getPropertyBook()), new UserPrefs());
