...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_ID_DESC, Id.MESSAGE_CONSTRAINTS); // invalid job ID
        assertParseFailure(parser, "1" + INVALID_TITLE_DESC, Title.MESSAGE_CONSTRAINTS); // invalid job title
        ...
        assertParseFailure(parser, "1" + INVALID_NAME_DESC + INVALID_EMAIL_DESC + VALID_ADDRESS_AMY
                + VALID_PHONE_AMY + VALID_ID_AMY + VALID_TITLE_AMY, Name.MESSAGE_CONSTRAINTS);
    }
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + TAG_DESC_KIV
                + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + TAG_DESC_REJECTED
                + ID_DESC_AMY + TITLE_DESC_AMY;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY)
                .withId(VALID_ID_AMY).withTitle(VALID_TITLE_AMY)
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        // job title
        userInput = targetIndex.getOneBased() + TITLE_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withTitle(VALID_TITLE_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    ...
    public void parse_multipleRepeatedFields_acceptsLast() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY
                + TAG_DESC_REJECTED + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY + TAG_DESC_REJECTED
                + PHONE_DESC_BOB + ADDRESS_DESC_BOB + EMAIL_DESC_BOB + TAG_DESC_KIV + ID_DESC_AMY
                + ID_DESC_BOB + TITLE_DESC_AMY + TITLE_DESC_BOB;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_BOB).withAddress(VALID_ADDRESS_BOB).withId(VALID_ID_BOB)
                .withTitle(VALID_TITLE_BOB).withTags(VALID_TAG_REJECTED, VALID_TAG_KIV).build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor);
    ...
    public void parse_invalidValueFollowedByValidValue_success() {
        ...
        userInput = targetIndex.getOneBased() + EMAIL_DESC_BOB + INVALID_PHONE_DESC + ADDRESS_DESC_BOB
                + PHONE_DESC_BOB + ID_DESC_BOB + TITLE_DESC_BOB;
        descriptor = new EditPersonDescriptorBuilder().withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_BOB)
                .withAddress(VALID_ADDRESS_BOB).withId(VALID_ID_BOB).withTitle(VALID_TITLE_BOB).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
