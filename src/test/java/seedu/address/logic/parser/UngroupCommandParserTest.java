...
public class UngroupCommandParserTest {
    ...
    public void parse_emptyArg_throwsParseException() {
        // missing group name
        assertParseFailure(parser, "2", String.format(MESSAGE_INVALID_COMMAND_FORMAT, UngroupCommand.MESSAGE_USAGE));
        // missing index
        assertParseFailure(parser, "ab", String.format(MESSAGE_INVALID_COMMAND_FORMAT, UngroupCommand.MESSAGE_USAGE));
        // missing group name with space
        assertParseFailure(parser, "2 ", String.format(MESSAGE_INVALID_COMMAND_FORMAT, UngroupCommand.MESSAGE_USAGE));
        // missing index with space
        assertParseFailure(parser, " ab", String.format(MESSAGE_INVALID_COMMAND_FORMAT, UngroupCommand.MESSAGE_USAGE));
        // invalid group name (has space)
        assertParseFailure(parser, "2 a b", String.format(MESSAGE_INVALID_COMMAND_FORMAT, UngroupCommand.MESSAGE_USAGE));
