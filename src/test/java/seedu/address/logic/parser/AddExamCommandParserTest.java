...
public class AddExamCommandParserTest {
    ...
    public void parse_compulsoryFieldMissingPrefix_failure() {
        ...
        assertParseFailure(parser, VALID_MODULE_EXAMONE + VALID_DESCRIPTION_EXAMONE + VALID_DATE_EXAMONE, expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid module
        assertParseFailure(parser, INVALID_MODULE + EXAMDESCRIPTIONONE + EXAMDATEONE, ModuleCode.MODULE_CODE_CONSTRAINTS);
        ...
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_FORMAT_EXAM_DATEONE, ExamDate.DATE_FORMAT_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_FORMAT_EXAM_DATETWO, ExamDate.DATE_FORMAT_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_FORMAT_EXAM_DATETHREE, ExamDate.DATE_FORMAT_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_FORMAT_EXAM_DATEFOUR, ExamDate.DATE_FORMAT_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_FORMAT_EXAM_DATEFIVE, ExamDate.DATE_FORMAT_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_FORMAT_EXAM_DATESIX, ExamDate.DATE_FORMAT_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_FORMAT_EXAM_DATESEVEN, ExamDate.DATE_FORMAT_CONSTRAINTS);
        //date is not valid
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_EXAM_DATEONE, ExamDate.VALID_DATE_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_EXAM_DATETWO, ExamDate.VALID_DATE_CONSTRAINTS);
        //date is a past date
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_PAST_EXAMDATEONE, ExamDate.NOT_A_PAST_DATE_CONSTRAINTS);
        assertParseFailure(parser, EXAMMODULEONE + EXAMDESCRIPTIONONE + INVALID_PAST_EXAMDATETWO, ExamDate.NOT_A_PAST_DATE_CONSTRAINTS);

        // three invalid values, only first invalid value reported
        assertParseFailure(parser, INVALID_MODULE + INVALID_EXAM_DESCRIPTION+ INVALID_EXAM_DATEONE,
                ModuleCode.MODULE_CODE_CONSTRAINTS);

        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + EXAMMODULEONE + EXAMDESCRIPTIONONE + EXAMDATEONE,
