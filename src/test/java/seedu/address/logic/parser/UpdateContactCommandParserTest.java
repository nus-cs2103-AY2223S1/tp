...
class UpdateContactCommandParserTest {
    ...
    void parse_assignNurseToPatient_fail() {
        Command setPhysCommand = new UpdateContactCommand(new Uid(3L), new Name(DEFAULT_CONTACT_NAME),
                new Phone(DEFAULT_CONTACT_PHONE), new Email(DEFAULT_CONTACT_EMAIL), new Category(Category.NURSE_SYMBOL));
        String input = UpdateContactCommand.COMMAND_WORD + " id/3 n/" + DEFAULT_CONTACT_NAME + " e/" + DEFAULT_CONTACT_EMAIL
                + " p/" + DEFAULT_CONTACT_PHONE + " c/" + Category.NURSE_SYMBOL;
    ...
    void parse_assignJohnDoeToPatient_success() {
        ...
        String input = UpdateContactCommand.COMMAND_WORD + " id/3 n/" + DEFAULT_CONTACT_NAME + " e/" + DEFAULT_CONTACT_EMAIL
                + " p/" + DEFAULT_CONTACT_PHONE + " c/" + Category.PHYSICIAN_SYMBOL;
        assertParseSuccess(new SetPhysicianCommandParser(), input, setPhysCommand);
    }
    ...
    void parse_assignJohnDoeNokToPatient_success() {
        Command setPhysCommand = new UpdateContactCommand(new Uid(3L), new Name(DEFAULT_CONTACT_NAME),
                new Phone(DEFAULT_CONTACT_PHONE), new Email(DEFAULT_CONTACT_EMAIL), new Category(Category.PHYSICIAN_SYMBOL));
        String input = UpdateContactCommand.COMMAND_WORD + " id/3 n/" + DEFAULT_CONTACT_NAME + " e/" + DEFAULT_CONTACT_EMAIL
                + " p/" + DEFAULT_CONTACT_PHONE + " c/" + Category.NEXTOFKIN_SYMBOL;
        assertParseSuccess(new SetPhysicianCommandParser(), input, setPhysCommand);
    }
}
