...
public class AddStudCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_BOB
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND, new AddStudCommand(expectedPerson));

        // multiple names - last name accepted
        assertParseSuccess(parser, STUDENT_NAME_DESC_AMY + STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_BOB
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND, new AddStudCommand(expectedPerson));

        // multiple phones - last phone accepted
        assertParseSuccess(parser, STUDENT_NAME_DESC_BOB + PHONE_DESC_AMY + PHONE_DESC_BOB + ID_DESC_BOB
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND, new AddStudCommand(expectedPerson));

        // multiple emails - last email accepted
        assertParseSuccess(parser, STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_AMY + ID_DESC_BOB
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND, new AddStudCommand(expectedPerson));

        // multiple addresses - last address accepted
        assertParseSuccess(parser, STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_BOB + ADDRESS_DESC_AMY
                + ADDRESS_DESC_BOB + TAG_DESC_FRIEND, new AddStudCommand(expectedPerson));

        // multiple tags - all accepted
        Student expectedPersonMultipleTags = new StudentBuilder(BOB).withTags(VALID_TAG_FRIEND, VALID_TAG_HUSBAND)
                .build();
        assertParseSuccess(parser, STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_BOB + ADDRESS_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, new AddStudCommand(expectedPersonMultipleTags));
    }
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        assertParseSuccess(parser, STUDENT_NAME_DESC_AMY + PHONE_DESC_AMY + ID_DESC_AMY + ADDRESS_DESC_AMY,
    ...
    public void parse_invalidValue_failure() {
        // invalid name
        assertParseFailure(parser, INVALID_STUDENT_NAME_DESC + PHONE_DESC_BOB + ID_DESC_BOB + ADDRESS_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Name.MESSAGE_CONSTRAINTS);

        // invalid phone
        assertParseFailure(parser, STUDENT_NAME_DESC_BOB + INVALID_PHONE_DESC + ID_DESC_BOB + ADDRESS_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Phone.MESSAGE_CONSTRAINTS);

        // invalid email
        assertParseFailure(parser, STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + INVALID_ID_DESC + ADDRESS_DESC_BOB
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Email.MESSAGE_CONSTRAINTS);

        // invalid address
        assertParseFailure(parser, STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_BOB + INVALID_ADDRESS_DESC
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Address.MESSAGE_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_BOB + ADDRESS_DESC_BOB
                + INVALID_TAG_DESC + VALID_TAG_FRIEND, Tag.MESSAGE_CONSTRAINTS);

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, INVALID_STUDENT_NAME_DESC + PHONE_DESC_BOB + ID_DESC_BOB + INVALID_ADDRESS_DESC,
                Name.MESSAGE_CONSTRAINTS);

        // non-empty preamble
        assertParseFailure(parser, PREAMBLE_NON_EMPTY + STUDENT_NAME_DESC_BOB + PHONE_DESC_BOB + ID_DESC_BOB
