...
public class ItemBoughtDateValidator implements Validator {
    ...
    private static final String MESSAGE_FOR_YEAR_TOO_SMALL =
            String.format("The year for item bought date should be larger than or equal to %d.", MIN_YEAR);

    private static final int MAX_YEAR = 2300;
    private static final String MESSAGE_FOR_YEAR_TOO_LARGE =
            String.format("The year for item bought date should be less than or equal to %d.", MAX_YEAR);
    ...
    public static Void validate(String dateString) {
        ...
        checkArgument(isDateStringInRightFormat, MESSAGE_FOR_BOUGHT_DATE_NOT_DDMMYYYY);
        checkArgument(isDateStringParsable, MESSAGE_FOR_BOUGHT_DATE_DO_NOT_EXIST);
