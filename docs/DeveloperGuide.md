...
Step 6. A new `Stall` object is created with the fields of `Review` to be edited, except the field(s) that are to be edited, which will be set by the parsed user input. In this case `Review` with index 2 is copied over to a new `Review` object expect its `Rating` field which is set as `5`.

Step 6. The `Stall` which contains the `Review` to be edited is copied to a new `Stall` object with all of its data fields except the `Review` to be edited which will be set by the parsed user input. In this case `Review` with index 2 is copied over to a new `Review` object expect its `Rating` field which is set as `5`.

Step 7. The new `Stall` object will then replace the `Stall` object that contains the `Review` to be edited in the `Model` when `model.setReview()` is called.
