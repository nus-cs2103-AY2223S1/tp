...
* `ResultDisplay` consists of `SessionListPanel`, each row representing a `SessionCard`,sorted based on the time it was added with the most recently added at the top, and `StudentListPanel`, each row representing a `StudentCard`, with their grades related to a particular session (given by user input) shown as well;
* `CommandBox`, the Command Line Interface (CLI) for user to key in command and
