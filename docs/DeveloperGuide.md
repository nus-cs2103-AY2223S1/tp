...
* Each ```Person```'s modules are stored as a separate _filtered_ list which is exposed to outsiders as an unmodifiable `ObservableList<Mod>` that can be 'observed' e.g. the UI can be bound to this list so that the UI automatically updates when the data in the list change.
