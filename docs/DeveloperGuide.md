...
* stores the address book data i.e., all `Exercise` objects (which are contained in a `UniqueExerciseList` object).
...
<div markdown="span" class="alert alert-info">:information_source: **Note:** An alternative (arguably, a more OOP) model is given below. It has a `Tag` list in the `AddressBook`, which `Exercise` references. This allows `AddressBook` to only require one `Tag` object per unique tag, instead of each `Exercise` needing their own `Tag` objects.<br>
