...
/**
 * Changes the remark of an existing teammate in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the teammate identified "
            + "by the index number used in the last teammate listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the teammate in the filtered teammate list to edit the remark
     * @param remark of the teammate to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of teammate data. We achieve that by working with the `Teammate` model. Each field in a Teammate is implemented as a separate class (e.g. a `Name` object represents the teammate’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a teammate.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.teammate`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each teammate.

Simply add the following to [`seedu.address.ui.TeammateCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
Then insert the following into [`main/resources/view/TeammateListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).

**`TeammateListCard.fxml`:**
...
## Modify `Teammate` to support a `Remark` field

Since `TeammateCard` displays data from a `Teammate`, we need to update `Teammate` to get our `Remark` displayed!

### Modify `Teammate`

We change the constructor of `Teammate` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Teammate`

Unfortunately, a change to `Teammate` will cause other commands to break, you will have to modify these commands to use the updated `Teammate`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Teammate` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedTeammate` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedTeammate` to work with our new `Teammate`!
...
Now that we have finalized the `Teammate` class and its dependencies, we can now bind the `Remark` field to the UI.
...
public TeammateCard(Teammate teammate, int displayedIndex) {
    //...
    remark.setText(teammate.getRemark().value);
}
```
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Teammate`. Since all fields in a `Teammate` are immutable, we create a new instance of a `Teammate` with the values that we want and
save it with `Model#setTeammate()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Teammate: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Teammate: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Teammate editedTeammate = new Teammate(
                teammateToEdit.getName(), teammateToEdit.getPhone(), teammateToEdit.getEmail(),
                teammateToEdit.getAddress(), remark, teammateToEdit.getTags());

        model.setTeammate(teammateToEdit, editedTeammate);
        model.updateFilteredTeammateList(PREDICATE_SHOW_ALL_TEAMMATES);

        return new CommandResult(generateSuccessMessage(editedTeammate));
    }
     ...
     * {@code teammateToEdit}.
     */
    private String generateSuccessMessage(Teammate teammateToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, teammateToEdit);
    }
```
