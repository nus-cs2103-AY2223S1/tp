...
This command allows users of the FRIDAY application to add optional remarks to people in their FRIDAY and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/masteryCheck/logic/command` directory.
...
``` java
package seedu.masteryCheck.logic.commands;

import Model;

/**
 * Changes the remark of an existing student in the FRIDAY.
 */
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `FridayParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
Create a `RemarkCommandParser` class in the `seedu.masteryCheck.logic.parser` package. The class must extend the `Parser` interface.
...
:information_source: Don’t forget to update `FridayParser` to use our new `RemarkCommandParser`!
...
Create a new `Remark` in `seedu.masteryCheck.model.student`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`StudentCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
