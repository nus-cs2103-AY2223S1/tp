...
/**
 * Changes the remark of an existing internship in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the internship identified "
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of internship data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the internship’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a internship.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each internship.

Simply add the following to [`seedu.address.ui.InternshipCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
``` java
public PersonCard(Person internship, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person internshipToEdit = lastShownList.get(index.getZeroBased());
        Person editedInternship = new Person(
                internshipToEdit.getName(), internshipToEdit.getPhone(), internshipToEdit.getEmail(),
                internshipToEdit.getAddress(), remark, internshipToEdit.getTags());
     ...
     */
    private String generateSuccessMessage(Person internshipToEdit) {
