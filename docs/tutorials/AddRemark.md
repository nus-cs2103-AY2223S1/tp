...
Then insert the following into [`main/resources/view/CustomerListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).

**`CustomerListCard.fxml`:**
...
## Modify `Customer` to support a `Remark` field

Since `CustomerCard` displays data from a `Customer`, we need to update `Customer` to get our `Remark` displayed!

### Modify `Customer`

We change the constructor of `Customer` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Customer`

Unfortunately, a change to `Customer` will cause other commands to break, you will have to modify these commands to use the updated `Customer`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Customer` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedCustomer` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedCustomer` to work with our new `Customer`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete AddressBook’s storage file located at `/data/artbuddy.json` before running it! Not doing so will cause AddressBook to default to an empty address book!
...
Now that we have finalized the `Customer` class and its dependencies, we can now bind the `Remark` field to the UI.
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Customer`. Since all fields in a `Customer` are immutable, we create a new instance of a `Customer` with the values that we want and
save it with `Model#setCustomer()`.
...
//...
     ...
     * {@code customerToEdit}.
     */
    private String generateSuccessMessage(Customer customerToEdit) {
