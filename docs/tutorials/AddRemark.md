...
/**
 * Changes the remark of an existing activity in the description book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the activity identified "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the activity in the filtered activity list to edit the remark
     * @param remark of the activity to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of activity data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the activity’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a activity.

### Add a new `Remark` class

Create a new `Remark` in `seedu.description.model.activity`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each activity.
...
``` java
public PersonCard(Person activity, int displayedIndex) {
    //...
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedActivity = new Person(
                activityToEdit.getName(), activityToEdit.getPhone(), activityToEdit.getEmail(),
                activityToEdit.getAddress(), remark, activityToEdit.getTags());

        model.setPerson(activityToEdit, editedActivity);
     ...
     * {@code activityToEdit}.
     */
    private String generateSuccessMessage(Person activityToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, activityToEdit);
    }
```
