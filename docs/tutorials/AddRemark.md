...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/taskbook/logic/command` directory.
...
Create a `RemarkCommandParser` class in the `taskbook.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `taskbook.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
