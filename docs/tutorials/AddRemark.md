...
/**
 * Changes the remarks of an existing student in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remarks of the student identified "
...
Now let’s move on to writing a parser that will extract the index and remarks from the input provided by the user.

Create a `RemarkCommandParser` class in the `seedu.watson.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.watson.model.student`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.watson.ui.StudentCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
