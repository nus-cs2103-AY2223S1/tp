...
1. The rest of the method seems to exhaustively check for the existence of each possible parameter of the `edit` command and store any possible changes in an `EditCustomerDescriptor`. Recall that we can verify the contents of `editCustomerDesciptor` through the 'Variables' window.<br>
...
1. As suspected, `command#execute()` does indeed make changes to the `model` object. Specifically,
   * it uses the `setCustomer()` method (defined in the interface `Model` and implemented in `ModelManager` as per the usual pattern) to update the customer data.
   * it uses the `updateFilteredCustomerList` method to ask the `Model` to populate the 'filtered list' with _all_ customers.<br>
...
1. It appears that a `JsonAdaptedCustomer` is created for each `Customer` and then added to the `JsonSerializableAddressBook`.
