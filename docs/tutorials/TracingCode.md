...
1. _Step into_ the line where user input in parsed from a String to a Command, which should bring you to the `FridayParser#parseCommand()` method (partial code given below):
   ``` java
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the FRIDAY in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Student`

    6.  Add a new entity to the FRIDAY
