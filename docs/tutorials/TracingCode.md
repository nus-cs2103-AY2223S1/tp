...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `logic.bookface.Logic`.
...
A quick look at the `logic.bookface.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save BookFace in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Person`

    6.  Add a new entity to BookFace
