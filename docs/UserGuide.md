...
2. Download the latest `cobb.jar` from [here](https://github.com/AY2223S1-CS2103T-F12-1/tp/releases).

3. Copy the file to the folder you want to use as the _home folder_ for Cobb.
...
Examples:<br>
`editprop 3 -n Hill Residence -a Block 225 -ph 750000`: Updates property at index 3 of the list to have a new name Hill Residence, a new address Block 225 and price 750000.
...
Note that unlike the version of the command above, the user is allowed to provide multiple filtering conditions.
The buyer list will be filtered to contain buyers who match *any one of the given commands*.

Examples:<br>
`filterbuyers -p 500000 -c bright; sunny -priority HIGH`: Filters all buyers that have a price range containing $500000 *OR* desired characteristics of bright and sunny *OR* a `HIGH` priority.
...
Syntax: `filterprops {-r PRICE RANGE} {-c CHARACTERISTICS} {-owner OWNER NAME}`
...
Note that unlike the version of the command above, the user is allowed to provide multiple filtering conditions.
The property list will be filtered to contain properties which match *any one of the given commands*.

Examples:<br>
`filterprops -r 500000-1000000 -c bright; sunny -owner GARY`: Filters all properties that have a price in the range $500000 - $1000000 _OR_ have characteristics of bright and sunny _OR_ is owned by someone named Gary.
...
Cobb's data are saved as 2 separate JSON files `[JAR file location]/data/personmodel.json` and `[JAR file location]/data/propertymodel.json`. Advanced users are welcome to update data directly by editing that data file.
