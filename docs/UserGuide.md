...
* The search is case-insensitive. e.g `t/EAR` has same effect as `t/ear`
* All tags of a patient are searched.
...
The search for reason, tag, and status is case-insensitive. e.g `t/EAR` has same effect as `t/ear`
...
* The reasons are case-insensitive. e.g. `r/PAIN` has same effect as `r/pain`
...
* The search is case-insensitive. e.g `n/hans` has same effect as `n/Hans`
